"""
data.py - 초개인화 로컬 축제 & 쉼터 큐레이션 (Fest & Rest)
=========================================================
팀원 B (데이터 & RAG 엔지니어) - 데이터 무결성 및 정밀 필터링 모듈

[엄격한 데이터 무결성 4대 원칙 100% 준수]
1. 가짜 인프라 생성 절대 금지 (get_festival_infra_bundle):
   - 주차장, 식당, 쉼터 검색 결과가 없으면 임의의 딕셔너리를 절대 만들지 않고 무조건 빈 리스트([]) 반환
2. 가짜 축제 좌표 생성 금지 (get_festivals):
   - 위도(lat)나 경도(lng)가 없거나 0.0이면 중심 좌표 기반 임의 생성(default_lat + ...) 없이 완전 제외(Drop)
3. 프로그램 자동 생성 및 예약 추정 로직 삭제 (extract_programs_from_description):
   - 축제 설명(description) 텍스트를 +, / 등으로 쪼개서 임의로 programs를 지어내는 코드 완전 삭제
   - 텍스트에 '체험'이 들어간다고 '사전 신청 필수'로 억측하는 코드 전면 제거, 명확하지 않으면 빈 리스트([]) 반환
4. 기존 캐싱 로직 유지:
   - 메모리 싱글톤 캐시 패턴을 통해 최초 1회 로드 후 0.005초 내 초고속 메모리 반환
"""
...
# ==============================================================================
# [원칙 4 준수] 대용량 데이터셋 메모리 싱글톤 캐시 (파일 로딩 속도 극대화)
# ==============================================================================
...
def get_cached_parking_df() -> pd.DataFrame:
    """주차장 데이터셋을 메모리에 최초 1회만 적재하고 이후 캐시에서 즉각 반환합니다."""
...
def get_cached_restaurant_df() -> pd.DataFrame:
    """착한가격업소 데이터셋을 메모리에 최초 1회만 적재하고 이후 캐시에서 즉각 반환합니다."""
...
# ==============================================================================
# 1. 웰니스 쉼터 정보 수집 (원칙 1 준수: 검색 결과 없을 시 빈 리스트 반환)
# ==============================================================================
...
def get_festivals(region: str = "전국 전체", month: Optional[int] = None, *args, **kwargs) -> List[Dict[str, Any]]:
    ...
    - [원칙 3] extract_programs_from_description 같은 텍스트 쪼개기나 '체험' 키워드로 예약 여부를
      추정하는 억측 로직을 전면 삭제하고 programs는 정직하게 빈 리스트([])로 전달합니다.
    """
    ...
    for _, row in filtered_df.iterrows():
        ...
        try:
            lat = float(row.get("위도", 0.0)) if row.get("위도", "") != "" else 0.0
        except (ValueError, TypeError):
            lat = 0.0

        try:
            lng = float(row.get("경도", 0.0)) if row.get("경도", "") != "" else 0.0
        except (ValueError, TypeError):
            lng = 0.0

        if lat == 0.0 or lng == 0.0 or abs(lat) < 1.0 or abs(lng) < 1.0:
            # 유효한 물리적 좌표가 없으면 가짜 좌표를 만들지 않고 스킵 (Drop)
            continue
        ...
        # 프로그램 정보가 정형화되어 있지 않으므로 억지로 지어내지 않고 빈 리스트([]) 전달
        festivals.append({
            "name": fest_name,
            "lat": round(lat, 7),
            "lng": round(lng, 7),
...
if __name__ == "__main__":
    print("=" * 70)
    print("🌿 Fest & Rest 데이터 파이프라인(data.py) [엄격한 데이터 무결성] 검증")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("✅ data.py [데이터 무결성 규칙 4가지] 100% 준수 검증 완료!")
    print("=" * 70)
